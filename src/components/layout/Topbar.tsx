...
export function Topbar() {
  ...
  return (
    <header className="h-16 bg-background flex items-center justify-between px-6 shrink-0 z-10 transition-colors">
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2 text-foreground font-semibold">
          <span className="font-serif text-[18.9px] font-medium tracking-tight">BatchRefiner</span>
        </div>
        
        <div className="flex items-center gap-2 text-text-secondary text-[14.7px] before:content-['/'] before:opacity-30 before:mr-2">
          
          <Popover open={isPopoverOpen} onOpenChange={handleOpenChange}>
            <PopoverTrigger render={
               <Button variant="ghost" className="h-auto p-1.5 -ml-1.5 hover:bg-black/5 text-text-primary px-2 text-[15.75px] rounded-lg flex items-center gap-1">
            ...
            <PopoverContent className="w-[300px] p-2 bg-card border-border shadow-lg rounded-2xl" align="start">
               <div className="flex flex-col gap-1">
                 <div className="px-2 py-1.5 flex justify-between items-center text-[11.55px] font-medium text-text-secondary">
                   ...
                   <Button variant="ghost" size="sm" className="h-6 px-2 text-[11.55px] bg-button-main/10 text-button-main hover:bg-button-main/20 rounded-md" onClick={handleCreateNew}>
                     <Plus className="w-3 h-3 mr-1" /> 新建项目
                   </Button>
                 </div>
                 
                 <div className="max-h-[300px] overflow-y-auto flex flex-col gap-1 mt-1">
                   {projects.length === 0 && <div className="p-3 text-[12.6px] text-text-secondary/50 text-center">暂无历史项目</div>}
                   {projects.map(p => (
                     <div 
                       key={p.projectId}
                     ...
                     >
                       <div className="flex flex-col flex-1 min-w-0 pr-2">
                         <div className="flex items-center gap-2">
                           <span className={`text-[13.65px] truncate ${p.projectId === projectId ? 'font-medium text-button-main' : 'text-text-primary'}`}>
                             {p.projectName}
                           </span>
