    ...
    <Popover>
      <PopoverTrigger render={
        <Button 
          variant="outline" 
          size="sm" 
          className={`h-8 bg-card border-border/80 text-[12.6px] font-mono text-text-primary hover:bg-black/5 ${triggerClassName}`}
        >
            ...
            <div className="flex gap-2">
              {RESOLUTIONS.map(res => (
                <button
                  ...
                  className={`flex-1 py-1.5 rounded-lg text-[12.6px] font-mono transition-colors border
                    ${resolution === res 
                      ? 'bg-button-main text-[#FFFFFF] border-button-main font-medium shadow-md' 
              ...
              {ASPECT_RATIOS.map(ar => (
                <button
                  ...
                  className={`flex flex-col items-center justify-center py-2 gap-1.5 rounded-xl border transition-all
                    ${aspectRatio === ar
                      ? 'bg-button-main/[0.03] border-button-main text-[#D97757] shadow-[0_0_0_1px_rgba(217,119,87,0.2)]'
                ...
                >
                  ...
                  <span className={`text-[11.55px] ${aspectRatio === ar ? 'font-medium' : 'opacity-80 font-mono'}`}>
                    {getLabel(ar)}
                  </span>
                </button>
              ))}
