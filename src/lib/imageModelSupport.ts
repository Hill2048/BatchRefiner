import type { AspectRatio, PlatformPreset, Resolution } from '@/types';
...
export function getComflyModelResolutionSupport(modelName: string, resolution: string): ResolutionSupport {
...
export function resolveComflyImageModel(modelName: string, resolution: string) {
  ...
  if (normalized.startsWith('nano-banana-pro')) {
    ...
    if (resolution === '4K') {
      throw new Error('comfly.chat 当前没有 nano-banana-pro 的 4K 模型，请改用 2K 或切换到 gemini-3.1-flash-image-preview 系列。');
    }
...
export function getGeminiSampleImageSize(resolution: string) {
  switch (resolution) {
    case '4K':
      return '4096';
    case '2K':
...
export function getOpenAIImageSize(aspectRatio: string) {
...
export function getImage2ImageSize(aspectRatio: string, resolution: string) {
...
export function getOpenAIImageQuality(resolution: string) {
  return resolution === '4K' || resolution === '2K' ? 'high' : 'medium';
}

export function parseCustomImageSize(resolution: string) {
...
export function isExactImage2Size(resolution: string) {
