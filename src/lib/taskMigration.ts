...
export function withDefaultSkill<T extends Partial<ProjectData> & { apiBaseUrl?: string }>(state: T) {
  ...
  return {
    ...
    textToImageApiBaseUrl: (state as T & { textToImageApiBaseUrl?: string }).textToImageApiBaseUrl || state.imageApiBaseUrl || state.apiBaseUrl,
    textToImageApiPath: (state as T & { textToImageApiPath?: string }).textToImageApiPath || (state as T & { imageApiPath?: string }).imageApiPath || DEFAULT_TEXT_TO_IMAGE_API_PATH,
    imageToImageApiBaseUrl: (state as T & { imageToImageApiBaseUrl?: string }).imageToImageApiBaseUrl || state.imageApiBaseUrl || state.apiBaseUrl,
    imageToImageApiPath: (state as T & { imageToImageApiPath?: string }).imageToImageApiPath || (state as T & { imageApiPath?: string }).imageApiPath || DEFAULT_IMAGE_TO_IMAGE_API_PATH,
    ...
    textToImageApiKey: (state as T & { textToImageApiKey?: string }).textToImageApiKey || (state as T & { imageApiKey?: string }).imageApiKey || legacyApiKey,
    imageToImageApiKey: (state as T & { imageToImageApiKey?: string }).imageToImageApiKey || (state as T & { imageApiKey?: string }).imageApiKey || legacyApiKey,
    textToImageModel: (state as T & { textToImageModel?: string }).textToImageModel || state.imageModel,
    imageToImageModel: (state as T & { imageToImageModel?: string }).imageToImageModel || state.imageModel,
  };
}
